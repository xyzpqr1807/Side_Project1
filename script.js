...
function loadDatabaseContent() {
  ...
  // Load VIP Shorts Feed (YouTube Embeds)
    if(typeof ourVideos !== 'undefined') {
        const shortsGallery = document.getElementById('shorts-gallery');
        if(shortsGallery) {
            ourVideos.forEach(videoId => {
                let post = document.createElement('div');
                post.className = 'short-post';
                
                // We use the 11-digit videoId to build the embed link!
                post.innerHTML = `
                    ...
                    <div class="short-comments">
                        <p><strong>@TheBirthdayQueen</strong> I loved making this one! ✨</p>
                        <p><span class="fan-comment">@SecretVIPFan (You)</span> Literally the best content creator on the internet. 😍🔥</p>
                    </div>
                `;
                shortsGallery.appendChild(post);
            });
        }
    }
    ...
    if (typeof ourMixtape !== 'undefined') {
        ...
        if(mixGallery) {
            ourMixtape.forEach(songId => {
                ...
                // We inject playsinline=1 so it never redirects to the YouTube app!
                iframe.src = `https://www.youtube.com/embed/${songId}?rel=0&modestbranding=1&playsinline=1`;
